...
class RecommendationService:
    ...
    def get_recommendations_for_user(self, user_id, limit=10):
        """
        Purely heuristic recommendation engine using multiple intent signals.
        """
    ...
    def _compute_weighted_recommendations(self, history, pool, limit, user_profile=None, search_history=None, map_history=None, already_initialized=False):
        """
        ...
        if not history.empty:
            ...
            for _, interaction in history.iterrows():
                ...
                property_pool.loc[matches, 'score'] += 0.5 * weight
                ...
                if interaction.get('village'):
                    ...
                    property_pool.loc[matches, 'score'] += 0.5 * weight
                    for idx in matches: add_to_breakdown(idx, 'local_niche_affinity', 0.5 * weight)
        ...
        if search_history is not None and not search_history.empty:
            ...
            if active_search_type == 'property':
                ...
                property_pool.loc[matches, 'score'] += 0.5
                for idx in matches: add_to_breakdown(idx, 'active_tab_intent', 0.5)
            ...
            for cat, val_lookup, val, index, search_type in flattened_specifics:
                ...
                if cat == 'listing_intent':
                    ...
                    matches = property_pool[property_pool['listingType'].apply(lambda x: lt_upper in [i.upper() for i in x] if isinstance(x, list) else (str(x).upper() == lt_upper))].index
                    boost = 1.0 * weight # UP from 0.8 - renting vs buying is a strict financial barrier
                ...
                if cat == 'bedroom_preference':
                    try:
                        ...
                        if '+' in beds_val:
                            ...
                            matches = property_pool[property_pool['bedrooms'] >= min_beds].index
                        else:
                            target_beds = int(beds_val)
                            matches = property_pool[property_pool['bedrooms'] == target_beds].index
                ...
                if cat == 'bath_preference':
                    try:
                        ...
                        if '+' in baths_val:
                            ...
                            matches = property_pool[property_pool['bathrooms'] >= min_baths].index
                        else:
                            target_baths = int(baths_val)
                            matches = property_pool[property_pool['bathrooms'] == target_baths].index
                ...
                if cat == 'type_affinity':
                    ...
                    matches = property_pool[property_pool['propertyType'].str.lower().str.strip() == target_type].index
                    boost = 0.7 * weight # Adjusted from 0.8
                    property_pool.loc[matches, 'score'] += boost
                ...
                if cat == 'fuel_tech_intent':
                    ...
                    matches = property_pool[property_pool['fuelType'].str.lower() == target_fuel].index
                ...
                if cat == 'drive_experience':
                    ...
                    matches = property_pool[property_pool['transmission'].str.lower() == target_trans].index
            ...
            for index, search in history_reset.iterrows():
                ...
                if filters.get('year') and isinstance(filters['year'], list) and len(filters['year']) == 2:
                    ...
                    if not (y_range[0] == 1990 and y_range[1] == 2025):
                        ...
                        matches = property_pool[(property_pool['year'] >= y_range[0]) & (property_pool['year'] <= y_range[1])].index
            ...
            for index, search in history_reset.iterrows():
                ...
                if p_range and isinstance(p_range, list) and len(p_range) == 2:
                    ...
                    if p_min is not None or p_max is not None:
                        ...
                        query = pd.Series([True] * len(property_pool), index=property_pool.index)
            ...
            for amenity, latest_idx in all_amenities_found.items():
                ...
                matches = property_pool[property_pool['amenities'].apply(lambda x: any(target_am == str(a).lower() for a in x) if isinstance(x, list) else False)].index
